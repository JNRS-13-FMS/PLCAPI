...
{
    [RoutePrefix("api/Omron")]
    public class OmronController : ApiController
    {
        static JsonConfigHelper _JsonConfigHelperOmron = new JsonConfigHelper(@"IpConfig/fcFins.json");
        string _IPConfigOmron = _JsonConfigHelperOmron["IPConfig"];
        protected readonly ILog log = LogManager.GetLogger("JNRSLogger");
        OmronFinsNet profinet = null;
        ...
        public IHttpActionResult GetState()
        {
            ...
            Console.WriteLine("Omron/GetState开始调用-> " + begintime.ToString("yyyy-MM-dd HH:mm:ss.fff"));
            log.Info("Omron/GetState开始调用-> " + begintime.ToString("yyyy-MM-dd HH:mm:ss.fff"));

            List<ParaConfig> lstParaConfig = new List<ParaConfig>();
            List<IPConfig> lstIPConfig = JsonConvert.DeserializeObject<List<IPConfig>>(_IPConfigOmron);
            try
            {
                foreach (IPConfig item in lstIPConfig)
                {
                    //连接
                    profinet = new OmronFinsNet();
                    profinet.IpAddress = item.ip;
                    profinet.Port = item.port;
                    profinet.SA1 = 180;
                    profinet.DA1 = 30;
                    profinet.DA2 = 0;
                    profinet.ByteTransform.DataFormat = HslCommunication.Core.DataFormat.CDAB;
