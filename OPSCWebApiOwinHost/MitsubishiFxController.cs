...
{
    [RoutePrefix("api/MitFx")]
    public class MitsubishiFxController : ApiController
    {
        static JsonConfigHelper _JsonConfigHelperMitFx = new JsonConfigHelper(@"IpConfig/fcMitFx.json");
        string _IPConfigMitFx = _JsonConfigHelperMitFx["IPConfig"];
        protected readonly ILog log = LogManager.GetLogger("JNRSLogger");
        MelsecA1ENet profinet = null;
        ...
        public IHttpActionResult GetState()
        {
            ...
            Console.WriteLine("MitFx/GetState开始调用-> " + begintime.ToString("yyyy-MM-dd HH:mm:ss.fff"));
            log.Info("MitFx/GetState开始调用-> " + begintime.ToString("yyyy-MM-dd HH:mm:ss.fff"));

            List<ParaConfig> lstParaConfig = new List<ParaConfig>();
            List<IPConfig> lstIPConfig = JsonConvert.DeserializeObject<List<IPConfig>>(_IPConfigMitFx);
            try
            {
                foreach (IPConfig item in lstIPConfig)
                {
                    //连接
                    profinet = new MelsecA1ENet();
